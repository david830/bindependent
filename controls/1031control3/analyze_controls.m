...
beadfile = [stem1031 'BEADS-3_H7_H07_P3.fcs'];
blankfile = [stem1031 'Blank-3_H12_H12_P3.fcs'];
channels{1} = Channel('Pacific Blue-A', 405, 0, 0);
channels{1} = setPrintName(channels{1}, 'Blue');
channels{1} = setLineSpec(channels{1}, 'b');
colorfiles{1} = [stem1031 'EBFP2-3_H9_H09_P3.fcs'];

channels{2} = Channel('PE-Tx-Red-YG-A', 561, 610, 20);
channels{2} = setPrintName(channels{2}, 'Red');
channels{2} = setLineSpec(channels{2}, 'r');
colorfiles{2} = [stem1031 'mkate-3_H8_H08_P3.fcs'];

channels{3} = Channel('FITC-A', 488, 515, 20);
channels{3} = setPrintName(channels{3}, 'Yellow');
channels{3} = setLineSpec(channels{3}, 'y');
colorfiles{3} = [stem1031 'EYFP-3_H10_H10_P3.fcs'];

colorpairfiles{1} = {channels{2}, channels{3}, channels{1}, [stem1031 'RBY-3_H11_H11_P3.fcs']};
colorpairfiles{2} = {channels{3}, channels{1}, channels{2}, [stem1031 'RBY-3_H11_H11_P3.fcs']};
colorpairfiles{3} = {channels{1}, channels{2}, channels{3}, [stem1031 'RBY-3_H11_H11_P3.fcs']};
CM = ColorModel(beadfile, blankfile, channels, colorfiles, colorpairfiles);
CM=set_bead_plot(CM, 2);
CM=set_bead_min(CM, 1.5);
