function prediction = predict_triple_cascade(induction, circuit, grain, verbosity)
...
mod_repressor1 = circuit.repressor1;
mod_repressor1.output = circuit.repressor1.output/grain;
mod_repressor2 = circuit.repressor2;
mod_repressor2.output = circuit.repressor2.output/grain;
mod_repressor3 = circuit.repressor3;
mod_repressor3.output = circuit.repressor3.output/grain;
...
stage3 = zeros(size(induction.output));
...
sequence = zeros([size(stage1) grain 4]);
for i=1:grain,
    ...
    [inc2 extra2 inter2 lo_ex_dist2 hi_ex_dist2] = project(stage1,induction.plasmids,mod_repressor1,i==1);
    stage2 = stage2*(1-decay) + inc2*amp*production_mod(i);
    
    [inc3 extra3 inter3 lo_ex_dist3 hi_ex_dist3] = project(stage2,induction.plasmids,mod_repressor2,i==1);
    stage3 = stage3*(1-decay) + inc3*amp*production_mod(i);

    [inc4 extra4 inter4 lo_ex_dist4 hi_ex_dist4] = project(stage3,induction.plasmids,mod_repressor3,i==1);
    output = output*(1-decay) + inc4*amp*production_mod(i);

    safe_range = safe_range & inter1 & inter2 & inter3 & inter4;
    lower_extrapolations = lower_extrapolations + (extra1<0) + (extra2<0) + (extra3<0) + (extra4<0);
    upper_extrapolations = upper_extrapolations + (extra1>0) + (extra2>0) + (extra3>0) + (extra4>0);
    
    max_lower_extrapolation = max(max_lower_extrapolation,max(lo_ex_dist1,max(lo_ex_dist2,max(lo_ex_dist3,lo_ex_dist4))));
    max_upper_extrapolation = max(max_upper_extrapolation,max(hi_ex_dist1,max(hi_ex_dist2,max(hi_ex_dist3,hi_ex_dist4))));
